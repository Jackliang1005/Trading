...
> **当前状态（2026-07-20）：不可用于生产交易。** 下列旧命令依赖的
> `trading_command_core.py` 当前不存在。Webhook 虽保留兼容路由，但不能据此
> 宣称已具备做 T 下单能力。新的分工、确认协议和上线门槛见
> `docs/t_trading_architecture_and_theory.md`。

| 指令前缀 | 说明 |
|---------|------|
| `T 状态` | 旧协议，当前不可用 |
| `T 开启 300475` | 旧协议，当前不可用，不得视为下单确认 |
| `T 设置 300475 ...` | 旧协议，当前不可用 |
